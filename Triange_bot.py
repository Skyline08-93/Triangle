...
def calc_triangle(p1, p2, p3, invert2):
    ...
    if not a_price or not (MIN_LIQUIDITY <= a_cost <= MAX_LIQUIDITY):
        return None
    ...
    if not b_price or not (MIN_LIQUIDITY <= b_cost <= MAX_LIQUIDITY):
        return None
    ...
    if not c_price or not (MIN_LIQUIDITY <= c_cost <= MAX_LIQUIDITY):
        return None
